...
type
  ...
  private
    ...
    function CalcRotateLogFileName (cNumBackup: Integer; cFileDate: string; cZipped: Boolean;
...
begin
  ...
  fMaxFileSizeInMB := 20;
  fDailyRotate := False;
  FDailyRotateFileDateFormat := 'yyyymmdd';
...
{$ENDIF}
  ...
  if fFileName.StartsWith ('.' + PathDelim) then
    fFileName := StringReplace (fFileName, '.' + PathDelim, TPath.GetDirectoryName(exepath) + PathDelim, [])
  else if ExtractFilePath (fFileName) = '' then
    fFileName := TPath.GetDirectoryName (exepath) + PathDelim + fFileName;

{$IFDEF MSWINDOWS}
  if fAutoFileName then
    fFileName := Format ('%s\%s_%d.log', [TPath.GetDirectoryName(fFileName),
      TPath.GetFileNameWithoutExtension(fFileName), GetCurrentProcessId]);
{$ENDIF}
  if fMaxFileSizeInMB > 0 then
    fLimitLogSize := fMaxFileSizeInMB * 1024 * 1024
  else
    fLimitLogSize := 0;
  ...
  try
    ...
    begin
      try
        ...
        on E: Exception do
          NotifyError (Format('Can''t rotate log file: %s', [E.message]));
      end;
      Exit;
    end;
    // writes header info
...
begin
  ...
  if iiProcessId in IncludedInfo then
    WriteToStream (Format('PID         : %d', [SystemInfo.ProcessId]));
  ...
  if iiOSVersion in IncludedInfo then
    WriteToStream (Format('OS version  : %s', [SystemInfo.OSVersion]));
  // {$IFDEF MSWINDOWS}
  if iiHost in IncludedInfo then
    WriteToStream (Format('Host        : %s', [SystemInfo.HostName]));
  if iiUserName in IncludedInfo then
    WriteToStream (Format('Username    : %s', [Trim(SystemInfo.UserName)]));
  // {$ENDIF}
  WriteToStream (Format('Started     : %s', [DateTimeToStr(Now(), FormatSettings)]));
{$IFDEF MSWINDOWS}
  if IsService then
    WriteToStream ('AppType     : Service')
  else if System.IsConsole then
    WriteToStream ('AppType     : Console');

  if IsDebug then
    WriteToStream ('Debug mode  : On');
{$ENDIF}
  WriteToStream (FillStr('-', 70));
end;
...
begin
  ...
  begin
    ...
    if fUnderlineHeaderEventType then
...
begin
  ...
  if Value.StartsWith ('.' + PathDelim) then
    fRotatedFilesPath := StringReplace (Value, '.' + PathDelim, TPath.GetDirectoryName(exepath) + PathDelim, [])
  else
    fRotatedFilesPath := Value;
end;

function TLogFileProvider.CheckNeedRotate: Boolean;
begin
  if ((fLimitLogSize > 0) and (fLogWriter.BaseStream.Size > fLimitLogSize)) or
    ((fDailyRotate) and (not IsSameDay(fFileCreationDate, Now()))) then
    Result := true
  else
    Result := False;
end;
